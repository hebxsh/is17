	...
	public class Reward
	{
		...
		public static function getitem(elev:int):String{
			...
			for (var k:int = 0;k<DataPool.getArr("sel").length;k++){
				if (int(DataPool.getArr("sel")[k].id)<20000){
					SqlDb.insert("bag",{id:DataPool.getArr("sel")[k].id,num:1});
				}else if(int(DataPool.getArr("sel")[k].id)<40000){
					SqlDb.insert("userskill",{id:DataPool.getArr("sel")[k].id,nowlevel:0,exp:0,useing:0});
				}else{
					SqlDb.insert("bag",{id:DataPool.getArr("sel")[k].id,num:1});
				}
			...
			for (var i:int = 0;i<itemArr[elev].length;i++){
				if(Math.random()*100<itemArr[elev][i]){
					if(Math.random()<.5){
						...
						SqlDb.select("book","sel","WHERE level="+i+" ORDER BY RANDOM() LIMIT 1;");
					}else if(Math.random()<.6){
